defmodule Task1 do
  ...
  def preprocess([string|push], strings, numbers) do
    case {string, numbers} do
      ...
      {[number|rest], []} -> preprocess(push, strings ++ [rest], numbers ++ [String.to_integer(number)])
      # uz tam neco je
      {[number|rest], [_|_]} ->
        index = index_to_put(numbers, String.to_integer(number))
        preprocess(push, List.insert_at(strings, index, rest), List.insert_at(numbers, index, String.to_integer(number)))
    end
  end
end
